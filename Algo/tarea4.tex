...
\begin{itemize}
    ...
    \textbf{Respuesta: }\\
    Resuelve el problema de multiplicar dos matrices (especialmente cuadradas) $(n \times n)$ en un tiempo más rapido, pasando de tener una complejidad de tiempo de $O(n^3)$, a una complejidad de aproximadamente $O(n^{log_2 7}) = O(n^{2.81})$ ya que solo realiza 7 multiplicaciones de matrices en lugar de 8 como normalmente se haría, lo que reduce el número de operaciones necesarias para completar la multiplicación de las matrices.\\
    Su tiempo de ejecución esta dado por la siguiente recurrencia:
    \[
    T(n) =
    \begin{cases}
    \Theta(n^3), & \text{si } n \text{ es pequeño} \\
    7T(n/2) + \Theta(n^2) , & \text{si } n \text{ es grande}
    \end{cases}
    \]
    ...
    La idea general del algoritmo es dividir cada una de las matrices (Sean $A$ y $B$) en 4 submatrices de tamaño $\frac{n}{2} \times \frac{n}{2}$, y luego realizar 7 multiplicaciones de estas submatrices utilizando combinaciones lineales de las mismas $(M_1, M_2, M_3, M_4, M_5, M_6, M_7)$, para finalmente combinar los resultados de estas multiplicaciones y mediante sumas y restas entre estas obtener cada uno de los elementos de la matriz resultante $(C_{11}, C_{12}, C_{21}, C_{22})$ (Manber, 1989).\\

    \item ¿Por qué es importante?\\
    ¿cuál sería la alternativa ”trivial” para el problema que se resuelve?\\
    \textbf{Respuesta:}
    El algoritmo de Strassen es importante porque reduce la complejidad de la multiplicación de matrices, ya que la alternativa trivial tiene una complejidad de $O(n^3)$, mientras que el algoritmo de Strassen tiene una complejidad de aproximadamente $O(n^{2.81})$, porque divide las matrices en submatrices más pequeñas y utiliza recursión para multiplicarlas, lo que reduce el número de multiplicaciones necesarias y aunque usa más sumas y restas, estas son operaciones menos costosas a comparación de las multiplicaciones. Este algoritmo puede ser usado por otros problemas que involucran matrices, como la inversión de matrices, cálculos de determinantes y la resolución de sistemas de ecuaciones lineales.\\
    La alternativa trivial para el problema de multiplicar matrices es el método tradicional, en el que cada elemento de la matriz resultado es calculado mediante la suma de los productos de los elementos correspondientes de las filas de la primera matriz y las columnas de la segunda matriz, lo que tiene una complejidad de tiempo de $O(n^3)$ para matrices de tamaño $n \times n$.

    \item Escribe el pseudocódigo\\
    \textbf{Entrada:} Dos matrices A y B\\
    \textbf{Salida:} Una matriz, resultado del producto entre A y B.\\
    \begin{algorithm}
        \caption{Strassen}
        \If{len(A) <= 2}{
            \Return fuerza\_bruta(A, B)\\
        }
        a, b, c, d = separar(A)\\
        e, f, g, h = separar(B)\\
        p1 = strassen(a + d, e + h)\\
        ...
        c21 = p6 + p2\\
        c22 = p5 + p1 - p6 - p7\\
        C $\gets$ nuevaMatriz(c11, c12, c21, c22)\\
        \Return C\\
    \end{algorithm}
    \item ¿Qué técnica de diseño de algoritmos utiliza?\\
    El algoritmo de Strassen utiliza la técnica de diseño de algoritmos: Divide y vencerás, ya que divide las matrices en submatrices de tamaño $n/2 \times n/2$ y después resuelve el problema de multiplicación de matrices en estas submatrices más pequeñas.
    \item Ilustra la ejecución del algoritmo (en papel) con un ejemplar pequeño.\\
    \includegraphics[width=16cm]{momos/strassen_1.jpeg}
    \includegraphics[width=16cm]{momos/strassen_2.jpeg}
\end{itemize}
...
\begin{enumerate}
    ...
    Es importante notar que la complejidad dada es pseudopolinomial, ya que se está haciendo el análisis sobre la magnitud de $monto$ y no sobre el
    tamaño de monto.
    ...
    Si se asume que en las formas distintas no importa el orden (combinaciones), tenemos el siguiente algoritmo:
    ($monto$, $M$, $n$)\\
    Donde $M$ es un arreglo que contiene las monedas con sus denominaciones y $n$ es el tamaño del arreglo.
    \begin{algorithm}
...
\begin{itemize}
    \item Manber, U. (1989). Introduction to Algorithms: A Creative Approach.

    \item Peláez Valdés, C. (2018). \textit{Estructuras de datos con Java moderno}. Las Prensas de Ciencias.
